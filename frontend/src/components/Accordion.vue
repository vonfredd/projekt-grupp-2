...
const accordionItems = ref([
    ...
    { id: 3, label: 'Add movie hall', inputId: 'add-movie-hall', placeholder: 'Enter hall name', value: '' }
]);
    ...
    <section v-for="item in accordionItems" :key="item.id" class="border-b border-gray-300">
      ...
      >
        <form @submit.prevent="switchAccordionEvent(item.id)" class="p-8 bg-gray-50 h-20 flex items-center">
          <input v-model="item.value" class="border-solid border border-black h-8 px-1" :id="item.inputId" :placeholder="item.placeholder" type="text" />
