...
describe("MovieProfileView", () => {
    ...
    beforeEach(() => {
        global.fetch = vi.fn((url) => {
            if (url === `http://localhost:9000/movies/27205`) {
        ...
        wrapper = mount(MovieProfileView, {
            ...
            global: {
                stubs: {
                    Schedules: { template: '<div>Stubbed Child Component</div>' }
                }
            }
        });
    ...
    it("verifies that Schedules component is stubbed", () => {
        expect(wrapper.html()).toContain("Stubbed Child Component");
    });
